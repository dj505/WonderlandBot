...
class Lockdown:
    ...
    async def lockdown(self, ctx, *, channels=""):
        """Lock message sending in the channel. Execs+ only."""
        try:
    ...
    async def softlock(self, ctx, *, channels=""):
        """Lock message sending in the channel, without the "disciplinary action" note. Execs+ only."""
        try:
            if len(ctx.message.channel_mentions) == 0:
                channels = [ctx.message.channel]
            else:
    ...
    async def unlock(self, ctx, *, channels=""):
        """Unlock message sending in the channel. Execs+ only."""
        try:
            if len(ctx.message.channel_mentions) == 0:
                channels = [ctx.message.channel]
            else:
