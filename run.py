...
description = """
Cheshire Cat bot by dj505#7040
"""
...
async def on_command_error(error, ctx):
    if isinstance(error, commands.errors.CommandNotFound):
        await bot.send_message(ctx.message.channel, "Sorry {}, but I can't find that command.".format(ctx.message.author.mention))
...
async def on_ready():
    ...
    for server in bot.servers:
        ...
        # channels
        bot.rules = discord.utils.get(server.channels, name="code-of-conduct")
        bot.announcements_channel = discord.utils.get(server.channels, name="coffee-news")
        bot.voice_channel = discord.utils.get(server.channels, name="General")
        bot.botcmds_channel = discord.utils.get(server.channels, name="cheshire-cabin")
        bot.boterr_channel = discord.utils.get(server.channels, name="bot-errors")
        bot.mods_channel = discord.utils.get(server.channels, name="royal-tea-party")
        bot.modlogs_channel = discord.utils.get(server.channels, name="bot-logs")

        # roles
        bot.mod_role = discord.utils.get(server.roles, name="hallo mod")
        bot.member_role = discord.utils.get(server.roles, name="Wonderlings")
        bot.inactive_role = discord.utils.get(server.roles, name="Inactive")
        ...
        if len(failed_addons) != 0:
            ...
            for f in failed_addons:
                msg += "\n{}: `{}: {}`".format(*f)
        await bot.send_message(bot.boterr_channel, msg)
